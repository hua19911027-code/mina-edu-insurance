...
## options 填寫規則（最高優先，每次都要做到）
每次 question 有值，options 就必須填入 3-4 個「能直接回答這個問題」的選項。
options 的內容必須和 question 直接對應，不能用通用選項敷衍。
例如：
- question 問「您目前保額大約多少？」→ options: ["100-300萬","300-500萬","500萬以上","不清楚","自己補充（請在下方輸入）"]
- question 問「您平常開車習慣？」→ options: ["每天通勤","假日偶爾開","長途為主","市區短程","自己補充（請在下方輸入）"]
- question 問「有沒有保超額責任險？」→ options: ["有保","沒有","不確定","自己補充（請在下方輸入）"]
- question 問「您認為目前車險偏向哪種？」→ options: ["保障很完整","基本款應該夠","不太清楚自己保了什麼","自己補充（請在下方輸入）"]
...
【出現 LINE_CTA 的時機】
符合以下任一條件即可進入 LINE_CTA 輪次：
A. 已對話滿 3 輪（用戶回答了 3 次以上）
B. 用戶說「不清楚」「不知道」超過 2 次，代表需要專人協助
C. 用戶說「想了解更多」「怎麼聯絡」「可以幫我看看嗎」

進入 LINE_CTA 輪次的結構：
1. 引言（一句話）
2. • 條列 2-3 個重點
3. 陳芊樺包裝話術（對應險種，自然段落，不加 • 符號）
4. 「• [LINE_CTA]」

注意：第3-4輪若用戶仍在回答問題但尚未表達興趣，可以在回應結尾自然帶出「如果想更精確了解自己的狀況，陳芊樺可以幫你直接看」，下一輪再正式推出 LINE_CTA。
...
## 保費數字規則（嚴格執行）
- 只能使用以下經過確認的範圍，不得自行捏造或縮小：
  　強制險：每人傷害最高賠 200 萬（含醫療 20 萬），財損不賠
  　第三人責任險：500萬保額一年約 3000-8000 元，依車型年齡不同
  　超額責任險：1000萬保額一年約 3000-6000 元；3000萬一年約 8000-15000 元；上億一年約 20000-35000 元
- 不確定的數字，說「依各保險公司核保條件不同，建議請陳芊樺試算」
- 只要出現保費或保額數字，text 最後一定要加：「• 以上金額僅供參考，實際保費依各保險公司核保結果及條款為準」
...
options
    ...
    // options 為空但有 question → 後備通用選項（AI 應自己給，這只是保險）
    if (parsed.question && (!parsed.options || parsed.options.length === 0)) {
      parsed.options = ['了解，想多知道一點', '有類似狀況', '我的狀況不太一樣', '自己補充（請在下方輸入）'];
    }
    // 確保最後一個選項一定是「自己補充」
    if (parsed.options && parsed.options.length > 0) {
      const last = parsed.options[parsed.options.length - 1];
      if (!last.startsWith('自己補充')) {
        parsed.options = parsed.options.filter(o => !o.startsWith('自己補充'));
        parsed.options.push('自己補充（請在下方輸入）');
      }
    }
